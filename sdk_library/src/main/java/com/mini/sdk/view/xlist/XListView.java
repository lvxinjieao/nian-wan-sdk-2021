package com.mini.sdk.view.xlist;
...
public class XListView extends ListView implements OnScrollListener {
	...
	private final static int PULL_LOAD_MORE_DELTA = 30; // when pull network_security_config >= 30px
	// at bottom, trigger
	// load more.
	...
	private void initWithContext(Context context, AttributeSet attrs) {
		...
		mHeaderViewContent = (RelativeLayout) mHeaderView
				.findViewById(Resources.getId(m_context, "xlistview_header_content"));
...
//        mHeaderView.onLoadingDrawableSet(imageDrawable);
	...
	public boolean isScrollStop() {
		Logs.e("isScrollStop:" + isScrollStop);
		return isScrollStop;
	}
	...
	/**
	 * enable or disable pull network_security_config load more feature.
	 *
	 * @param enable
	 */
	public void setPullLoadEnable(boolean enable) {
		...
		if (!mEnablePullLoad) {
			...
			// both "pull network_security_config" and "click" will invoke load more.
		}
	}
	...
	private void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
			// SuningLoge( mHeaderView.getVisiableHeight() + ">" + mHeaderViewHeight +
...
//        Logs.e("height = " + height + " --" + PULL_LOAD_MORE_DELTA);
...
//                newScrollValue = Math.round(Math.max(mLastY - mLastAniY, 0) / FRICTION);
			// SuningLogs.e("XListViewHeader", "mLastY = " + mLastY + " mLastAniY = " +
			// mLastAniY);
			// SuningLogs.e("XListViewHeader", "newScrollValue = " + newScrollValue);
...
//                    mHeaderView.onPullImpl(scale);
				...
				// last item, already pulled network_security_config or want to pull network_security_config.
				updateFooterHeight(-deltaY / OFFSET_RADIO);
			}
			break;
		default:
//                Logs.e("LastVisiblePosition = " + getLastVisiblePosition() + " mTotalItemCount = "
//                        + mTotalItemCount);
			mLastY = -1; // reset
			if (getFirstVisiblePosition() == 0) {
				...
				if (mEnablePullLoad) {
//                        Logs.e("BottomMargin = " + mFooterView.getBottomMargin());
				...
				// invoke load more.
//                    Logs.e("mEnablePullLoad = " + mEnablePullLoad + " footScroll = " + footScroll);
				if (mEnablePullLoad) {
					if (mFooterView.getBottomMargin() > PULL_LOAD_MORE_DELTA) {
						startLoadMore();
					}
