package com.mini.sdk.db.comque;

import com.mini.sdk.db.BaseDBAdapter;
...
public class CommonQuesstionAdapter extends BaseDBAdapter {
	...
	public static final String DB_CommonQuesstionCREATE = "" + "create table " + tb_commonquesstion + " (" + key_id
			+ " integer primary key autoincrement," + key_cqid + " VARCHAR not null," + key_title + " VARCHAR ,"
	...
	public long AddComQue(CommonQuesstionBiz biz) {
		...
		if (biz.list.size() > 0) {
			...
			for (CommonQuesstionBiz cbiz : biz.list) {
				System.out.println("插入" + cbiz.toString());
	...
	public CommonQuesstionBiz getComQue() {
		Cursor cursor = null;
		try {
			cursor = db.query(tb_commonquesstion, new String[] { key_cqid, key_title, key_description }, null, null,
					null, null, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
