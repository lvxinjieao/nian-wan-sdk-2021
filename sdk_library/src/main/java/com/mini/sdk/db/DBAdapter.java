...
public class DBAdapter {
	...
	public static final String DB_CREATE = "create table " + DB_TABLE + "(" + KEY_ID
			+ " integer primary key autoincrement, " + KEY_ACCOUNT + " CHAR not null, " + KEY_PASSWORD + " CHAR);";
	...
	/**
	 * 通过 SQLiteDatabase 对象直接创建数据库方法如下： 首先调用 openOrCreateDatabases()方法 创建数据 库对象，然后执行
	 * SQL 命令建立数据库中的表和直接的关系
	 */
	public void create() {
	}
	...
	public UserInfo getOneData(String account) throws Exception {
		Cursor results = db.query(DB_TABLE, new String[] { KEY_ID, KEY_ACCOUNT, KEY_PASSWORD },
				KEY_ACCOUNT + "=" + account, null, null, null, null);
	...
	public UserInfo[] getAllData() {
		Cursor results = null;
		try {
			results = db.query(DB_TABLE, new String[] { KEY_ID, KEY_ACCOUNT, KEY_PASSWORD }, null, null, null, null,
					null);
		} catch (Exception e) {
			e.printStackTrace();
		}
